    ...
    for (int i = 0; i < num; i++) {
        switch ((data + i)->type) {
            case e_n: {
                ...
                int len_buffer = strlen((data + i)->name) + 1 + 1 + size_floor + 1;
            ...
            case e_d: {
                ...
                int len_buffer = strlen((data + i)->name) + 1 + 1 + (size_floor + (trailing_zeros != 0 ? 1 : 0) + trailing_zeros) + 1;
                char *buffer = malloc((len_buffer + 1) * sizeof(char));
...
char *ou_comparing_difficulty(ComparingDifficulty *data, unsigned int num) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        int len_buffer = strlen((data + i)->name) + 1 + (size_floor + (trailing_zeros != 0 ? 1 : 0) + trailing_zeros) + 1;
        char *buffer = malloc((len_buffer + 1) * sizeof(char));
        ...
        switch ((data + i)->type) {
            case g_cp:
                ...
                if ((*(data + i)->info.cp != NULL && *(data + i)->original.cp == NULL)
                    ...
                    int len_buffer = strlen((data + i)->name) + 1 + 1 + strlen(*(data + i)->info.cp) + 1;
                    char *buffer = malloc((len_buffer + 1) * sizeof(char));
...
char *ou_comparing_metadata(ComparingMetadata *data, unsigned int num) {
    ...
    for (int i = 0; i < num; i++) {
        switch ((data + i)->type) {
            case m_cp: {
                int len_buffer = strlen((data + i)->name) + 1 + 1;
                char *buffer = malloc((len_buffer + 1) * sizeof(char));
                snprintf(buffer, len_buffer, "%s:", (data + i)->name);
            ...
            case m_n: {
                ...
                int len_buffer = strlen((data + i)->name) + 1 + size_floor + 1;
                char *buffer = malloc((len_buffer + 1) * sizeof(char));
                snprintf(buffer, len_buffer, "%s:%d\n", (data + i)->name, *(data + i)->info.n);
